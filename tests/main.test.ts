...
describe('onReceberMensagem', () => {
  it('segue o fluxo de manutenção preventiva solicitando placa e quilometragem', async () => {
    ...
    expect(start.some((text) => text.includes('Por favor, informe a placa'))).toBe(true)
    ...
    expect(informaKm.some((text) => text.includes('Como podemos ajudar hoje?'))).toBe(true)

    const selecionaManutencao = await onReceberMensagem('2')
    expect(selecionaManutencao.some((text) => text.includes('Registramos o veículo ABC1D23'))).toBe(true)
    expect(selecionaManutencao.some((text) => text.includes('Deseja continuar o atendimento'))).toBe(true)
  ...
  it('retorna telefones para atendimento urgente', async () => {
    ...
    await onReceberMensagem('ABC1D23')
    await onReceberMensagem('45000')
